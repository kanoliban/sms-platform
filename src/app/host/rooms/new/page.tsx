...
const toneConfig: Record<RoomTone, { label: string; description: string; gradient: string; color: string }> = {
  chill: {
    label: 'Chill',
...
export default function CreateRoom() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/host/rooms/${data.room.id}`);
    } catch (err) {
  ...
  const FormPanel = (
    <form onSubmit={handleSubmit} className="space-y-8">
      {/* Room Name - Large, prominent input */}
      <div>
        <Input
          value={form.name}
      ...
      {showDescription ? (
        <div>
          ...
          <Textarea
            ...
            placeholder="What will this room be about? What should guests expect?"
            rows={3}
          />
        </div>
      ) : (
      ...
      <div>
        ...
        <p className="text-[var(--text-xs)] text-[var(--text-muted)] mb-3">
          This helps match guests with the right energy for your room.
        </p>
        <div className="grid grid-cols-2 gap-3">
          {(Object.keys(toneConfig) as RoomTone[]).map((tone) => (
            <button
              key={tone}
              type="button"
      ...
      <div>
        ...
        <p className="text-[var(--text-xs)] text-[var(--text-muted)] mb-3">
          Full address is revealed to guests 24 hours before the room.
        </p>
        <Input
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-base)]">
        ...
        <PageContainer>
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-4">
              ...
              <span className="text-[var(--text-secondary)]">Create Room</span>
            </div>
            <Link
              href="/host"
      ...
      <PageContainer className="py-8">
        <div className="mb-8">
          <h1 className="text-[var(--text-3xl)] font-semibold text-[var(--text-primary)]">Create a Room</h1>
          <p className="text-[var(--text-secondary)] mt-2">
            A room is a bounded space in time where strangers meet with intention.
          </p>
        </div>
