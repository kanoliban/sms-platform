...
function RoomSettingsContent() {
  ...
  const [tone, setTone] = useState<SpaceTone>('deep');
  ...
  function populateForm(space: SpaceWithHost) {
    setName(space.name);
    setDescription(space.description || '');
    setTone(space.tone as SpaceTone);
    setDate(space.date);
    setTime(space.time);
    setDuration(space.duration_minutes);
    setCapacity(space.capacity);
    setPrice(space.price_cents / 100);
    setLocationAddress(space.location_address);
    setLocationHint(space.location_hint || '');
    setLocationRevealed(space.location_revealed);
  }
  ...
  async function handleCancel() {
    if (!confirm('Are you sure you want to cancel this space? This cannot be undone.')) {
      return;
    }
        ...
        <div className="space-y-8">
          ...
          <Card className="p-6 border-[var(--error)] border-opacity-50">
            ...
            <p className="text-[var(--text-sm)] text-[var(--text-secondary)] mb-4">
              Cancelling this space will notify all guests and refund any payments.
