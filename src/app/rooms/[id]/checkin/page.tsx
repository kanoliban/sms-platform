...
// Tone configuration
const toneConfig: Record<RoomTone, { gradient: string }> = {
...
export default function CheckInPage() {
  ...
  async function handleCheckIn() {
    ...
    try {
      const response = await fetch(`/api/rooms/${roomId}/checkin`, {
        method: 'POST',
                  ...
                  <div className="mt-6 p-4 bg-[var(--bg-subtle)] rounded-[var(--radius-lg)]">
                    <div className="flex items-center gap-3 text-[var(--text-sm)]">
                      ...
                      <span className="text-[var(--text-secondary)]">
                        {room.location_revealed ? room.location_address : room.location_hint}
                      </span>
                    </div>
                  </div>
