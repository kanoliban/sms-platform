...
export function ShareRoomModal({ open, onClose, room }: ShareRoomModalProps) {
  const [copied, setCopied] = useState(false);

  const getShareUrl = useCallback((platform: SharePlatform): string | null => {
    const text = `Join me at ${room.name}!`;
      ...
      try {
        ...
        if ((err as Error).name !== 'AbortError') {
          toast({
            variant: 'error',
            title: 'Share failed',
            description: 'Could not share the room.',
          });
        }
      }
    }
  }, [room.name, room.url, toast]);
  ...
  return (
    <Modal
      ...
      description={`Invite people to ${room.name}`}
    >
